...
IMPLEMENT_DYNAMIC(BtnOne, CButton)
...
BEGIN_MESSAGE_MAP(BtnOne, CButton)
	ON_WM_MOUSEMOVE()
	ON_WM_MOUSELEAVE()
	ON_WM_MOUSEHOVER()
	ON_WM_LBUTTONDOWN()
...
void BtnOne::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	//1.加载图片
	CBitmap bitmap1, bitmap2, bitmap3;
	bitmap1.LoadBitmap(IDB_BITMAP1);										//正常时候的图片样子
	bitmap2.LoadBitmap(IDB_BITMAP2);										//悬停时候的图片样子
	bitmap3.LoadBitmap(IDB_BITMAP2);										//按下时候的图片样子//2.获取DC和内存DC
	//	2.1 获取绘图DC
...
void BtnOne::OnMouseMove(UINT nFlags, CPoint point)
{
...
void BtnOne::OnMouseHover(UINT nFlags, CPoint point)
{
	//一直在按钮上方，则不重复触发
...
void BtnOne::OnLButtonDown(UINT nFlags, CPoint point)
{
...
void BtnOne::OnLButtonUp(UINT nFlags, CPoint point)
{
