...
void bwn::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	//1.加载图片
	CBitmap bitmap1, bitmap2, bitmap3;
	bitmap1.LoadBitmap(IDB_BITMAP28);										//正常时候的图片样子
	bitmap2.LoadBitmap(IDB_BITMAP29);										//悬停时候的图片样子
	bitmap3.LoadBitmap(IDB_BITMAP29);										//按下时候的图片样子//2.获取DC和内存DC
	//	2.1 获取绘图DC
...
void bwn::OnMouseMove(UINT nFlags, CPoint point)
{
...
void bwn::OnMouseHover(UINT nFlags, CPoint point)
{
	//一直在按钮上方，则不重复触发
...
void bwn::OnLButtonDown(UINT nFlags, CPoint point)
{
...
void bwn::OnLButtonUp(UINT nFlags, CPoint point)
{
