...
IMPLEMENT_DYNAMIC(BtnDeclear, CButton)



BtnDeclear::BtnDeclear()
{
	m_btnState = NORMAL;
}
...
BEGIN_MESSAGE_MAP(BtnDeclear, CButton)
	ON_WM_MOUSEMOVE()
	ON_WM_MOUSELEAVE()
	ON_WM_MOUSEHOVER()
	ON_WM_LBUTTONDOWN()
...
void BtnDeclear::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	//1.加载图片
	CBitmap bitmap1, bitmap2, bitmap3;
	bitmap1.LoadBitmap(IDB_BITMAP41);										//正常时候的图片样子
	bitmap2.LoadBitmap(IDB_BITMAP42);										//悬停时候的图片样子
	bitmap3.LoadBitmap(IDB_BITMAP42);										//按下时候的图片样子//2.获取DC和内存DC
	//	2.1 获取绘图DC
...
void BtnDeclear::OnMouseMove(UINT nFlags, CPoint point)
{
...
void BtnDeclear::OnMouseHover(UINT nFlags, CPoint point)
{
	//一直在按钮上方，则不重复触发
...
void BtnDeclear::OnLButtonDown(UINT nFlags, CPoint point)
{
...
void BtnDeclear::OnLButtonUp(UINT nFlags, CPoint point)
{
