...
void Min::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	//1.加载图片
	CBitmap bitmap1, bitmap2, bitmap3;
	bitmap1.LoadBitmap(IDB_BITMAP11);										//正常时候的图片样子
	bitmap2.LoadBitmap(IDB_BITMAP12);										//悬停时候的图片样子
	bitmap3.LoadBitmap(IDB_BITMAP12);										//按下时候的图片样子//2.获取DC和内存DC
	//	2.1 获取绘图DC
...
void Min::OnMouseMove(UINT nFlags, CPoint point)
{
...
void Min::OnMouseHover(UINT nFlags, CPoint point)
{
	//一直在按钮上方，则不重复触发
...
void Min::OnLButtonDown(UINT nFlags, CPoint point)
{
...
void Min::OnLButtonUp(UINT nFlags, CPoint point)
{
