...
IMPLEMENT_DYNAMIC(recentFile, CButton)
...
BEGIN_MESSAGE_MAP(recentFile, CButton)
	ON_WM_MOUSEMOVE()
	ON_WM_MOUSELEAVE()
	ON_WM_MOUSEHOVER()
	ON_WM_LBUTTONDOWN()
...
void recentFile::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	//1.加载图片
	CBitmap bitmap1, bitmap2, bitmap3;
	bitmap1.LoadBitmap(IDB_BITMAP26);										//正常时候的图片样子
	bitmap2.LoadBitmap(IDB_BITMAP27);										//悬停时候的图片样子
	bitmap3.LoadBitmap(IDB_BITMAP27);										//按下时候的图片样子//2.获取DC和内存DC
	//	2.1 获取绘图DC
	...
	pDc->StretchBlt(0, 0, 120, 100, &cMemDc, 0, 0, 120, 100, SRCCOPY);
}


void recentFile::OnMouseMove(UINT nFlags, CPoint point)
{
...
void recentFile::OnMouseHover(UINT nFlags, CPoint point)
{
	//一直在按钮上方，则不重复触发
...
void recentFile::OnLButtonDown(UINT nFlags, CPoint point)
{
...
void recentFile::OnLButtonUp(UINT nFlags, CPoint point)
{
