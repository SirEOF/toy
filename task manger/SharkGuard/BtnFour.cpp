...
IMPLEMENT_DYNAMIC(BtnFour, CButton)
...
BEGIN_MESSAGE_MAP(BtnFour, CButton)
	ON_WM_MOUSEMOVE()
	ON_WM_MOUSELEAVE()
	ON_WM_MOUSEHOVER()
	ON_WM_LBUTTONDOWN()
...
void BtnFour::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	//1.加载图片
	CBitmap bitmap1, bitmap2, bitmap3;
	bitmap1.LoadBitmap(IDB_BITMAP5);										//正常时候的图片样子
	bitmap2.LoadBitmap(IDB_BITMAP17);										//悬停时候的图片样子
	bitmap3.LoadBitmap(IDB_BITMAP17);										//按下时候的图片样子//2.获取DC和内存DC
	//	2.1 获取绘图DC
...
void BtnFour::OnMouseMove(UINT nFlags, CPoint point)
{
...
void BtnFour::OnMouseHover(UINT nFlags, CPoint point)
{
	//一直在按钮上方，则不重复触发
...
void BtnFour::OnLButtonDown(UINT nFlags, CPoint point)
{
...
void BtnFour::OnLButtonUp(UINT nFlags, CPoint point)
{
