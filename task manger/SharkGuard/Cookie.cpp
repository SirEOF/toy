...
IMPLEMENT_DYNAMIC(Cookie, CButton)
...
BEGIN_MESSAGE_MAP(Cookie, CButton)
	ON_WM_MOUSEMOVE()
	ON_WM_MOUSELEAVE()
	ON_WM_MOUSEHOVER()
	ON_WM_LBUTTONDOWN()
...
void Cookie::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	//1.加载图片
	CBitmap bitmap1, bitmap2, bitmap3;
	bitmap1.LoadBitmap(IDB_BITMAP30);										//正常时候的图片样子
	bitmap2.LoadBitmap(IDB_BITMAP31);										//悬停时候的图片样子
	bitmap3.LoadBitmap(IDB_BITMAP31);										//按下时候的图片样子//2.获取DC和内存DC
	//	2.1 获取绘图DC
...
void Cookie::OnMouseMove(UINT nFlags, CPoint point)
{
...
void Cookie::OnMouseHover(UINT nFlags, CPoint point)
{
	//一直在按钮上方，则不重复触发
...
void Cookie::OnLButtonDown(UINT nFlags, CPoint point)
{
...
void Cookie::OnLButtonUp(UINT nFlags, CPoint point)
{
