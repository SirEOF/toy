...
MD5VAL CMD5::md5(char * str, unsigned int size /*= 0*/)
{
...
// 直接计算文件MD5
char * CMD5::md5FileValue(char * fname)
{
...
/* 获得字串的MD5值 */
void CMD5::md5Str(char * input, char * output)
{
	MD5VAL val;
	val = md5(input, strlen(input));
	sprintf_s(output, 32, "%08x%08x%08x%08x", conv(val.a), conv(val.b), conv(val.c), conv(val.d));
