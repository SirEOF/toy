...
MTraffic::MTraffic()
{
	lasttraffic = 0.0;
	CurrentInterface = -1;
	CurrentTrafficType = Traffic_AllTraffic;
	GetInterfaces();
}
...
double MTraffic::GetTraffic(int interfaceNumber)
{
	try
	{
...
//返回安装的接口数
int MTraffic::GetNetworkInterfacesCount()
{
...
//返回接口名称
BOOL MTraffic::GetNetworkInterfaceName(CString *InterfaceName, int index)
{
...
//返回接口带宽
DWORD MTraffic::GetInterfaceBandwidth(int index)
{
...
//具体接口接收和发送的数据量
DWORD MTraffic::GetInterfaceTotalTraffic(int index)
{
...
void MTraffic::SetTrafficType(int trafficType)
{
